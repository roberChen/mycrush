...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"glob", "grep", "ls", "sourcegraph", "view", "read_badge"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "crush_info", "crush_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "glob", "ls", "sourcegraph", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource", "read_badge", "update_badge"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, []string{"glob", "ls", "sourcegraph", "view", "read_badge"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "crush_info", "crush_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "todos", "write", "list_mcp_resources", "read_mcp_resource", "update_badge"}, coderAgent.AllowedTools)
