...
func (w *AppWorkspace) GetInheritedMessageCount(sessionID string) int {
	sess, err := w.app.Sessions.Get(context.Background(), sessionID)
	if err != nil {
		return 0
	}
	return int(sess.InheritedMessageCount)
}
